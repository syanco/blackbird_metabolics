...
pred <- cbind(nd, p) %>% 
  mutate(tot_min_therm = p-metab,
         ...
         strat = fct_relevel(strat, rev(levels(strat)))) 
...
p4a <- ggplot(pred) + 
  geom_line(aes(x=as.factor(julian.bird), y=metab, color = strat, group = logger.id), alpha = 0.4) +
  scale_color_manual(values = pal, labels = c(fm = "Migrants", ws = "Residents"),
                     name = "Migratory Strategy") +
