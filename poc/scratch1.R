...
# heat balance
enbal.fm <- endo.out.fm[, grep(pattern = "enbal", colnames(endo.out.fm))]
colnames(enbal.fm) <- gsub(colnames(enbal.fm), pattern = "enbal.", 
                           replacement = "")
enbal.ws <- endo.out.ws[, grep(pattern = "enbal", colnames(endo.out.ws))]
colnames(enbal.ws) <- gsub(colnames(enbal.ws), pattern = "enbal.", 
                           replacement = "")

# PLOTS #

# Plot body temp by time
plot(1:length(fmtemp), fmtemp, type = "l", ylim = c(40.5, 41.8),
     ylab = "Body Temp. (C)", xlab = "Julian Bird")
lines(1:length(wstemp), wstemp, col = "red")


# Plot ambient temp by time
plot(1:length(TAfm), TAfm, type = "l", ylim = c(-1, 20),
...
plot(1:length(hrfm), hrfm, type = "l", ylim = c(370,490),
...
plot(1:length(fmtemp), enbal.fm$QGEN, type = "l", ylim = c(1.3, 2.5),
     ylab = "Metabolic Rate (W)", xlab = "Julian Bird")
lines(1:length(wstemp), enbal.ws$QGEN, col = "red")

# Met rate by temp body temp
plot(fmtemp, enbal.fm$QGEN, ylim= c(1.3, 2.5), xlim = c(40.5,41.8),
