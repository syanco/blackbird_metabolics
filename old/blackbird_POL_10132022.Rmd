...
### Body Mass at Arrival
**N.B. - Below considers body mass to understand if arrival condition in spring is equivalent (i.e., can the excess metabolic activity in the migrants be attributed to tissue anabolism? BUT, it's poorly controlled for sampling date so, for now, is more of an idea than a result... we may want to skip this entirely**
...
```{r fig.cap = "Breeding season body condition (bodymass/tarsus length) by sex and migratory strategy"}
ggplot(bodymass1) +
...
*My $0.02:  I think it would be nice to be able to point quantitatively to where we expect this energy to go - for which the DEB might be nice.  That said, I suspect it might be very difficult to sensibly parameterize the DEB because the costs of migration itself are going to be very difficult to capture, especially those periods of tissue hypertrophy and atrophy that pre-/ante-cede migrations themselves and without those quantitatively account for, I'm not positive the DEB is going to be any more convincing than reasoning by logic alone... (see the bullet points in the discussion as of now). Also, the `nichmapr` DEb extension for endotherms isn't yet ready so this may be moot anywasy (i.e., not possible).*
